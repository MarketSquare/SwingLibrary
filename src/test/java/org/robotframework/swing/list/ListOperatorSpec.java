...
public class ListOperatorSpec extends MockSupportSpecification<ListOperator> {
    public class Any {
        ...
        public void selectsItem() {
            checking(new Expectations() {
                {
                    atLeast(1).of(textExtractor).itemCount();
                    will(returnValue(4));
                    ignoring(textExtractor).getTextFromRenderedComponent(0);
                    will(returnValue("one"));
                    ignoring(textExtractor).getTextFromRenderedComponent(1);
                    will(returnValue("2"));
                    ignoring(textExtractor).getTextFromRenderedComponent(2);
                    will(returnValue("3"));
                    ignoring(textExtractor).getTextFromRenderedComponent(3);
        ...
        public void clicksOnListItem() {
            ...
            checking(new Expectations() {
                {
                    ignoring(textExtractor).itemCount();
                    will(returnValue(8));
                    one(jListOperator).clickOnItem(5, 1);
                    one(jListOperator).clickOnItem(0, 2);
                    ignoring(textExtractor).getTextFromRenderedComponent(
                            with(any(Integer.class)));
                    will(returnValue(items[0]));
